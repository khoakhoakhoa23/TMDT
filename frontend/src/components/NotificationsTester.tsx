...
const NotificationsTester: React.FC = () => {
  ...
  const connectNotifications = () => {
    ...
    ws.onopen = () => {
      setMessages((m) => [
        ...m,
        { type: "system", text: "Connected to notifications WS" },
      ]);
    };
    ws.onmessage = (ev: MessageEvent) => {
      try {
        ...
        setMessages((m) => [
          ...m,
          { type: data.type || "msg", text: JSON.stringify(data) },
        ]);
      } catch (e) {
    ...
    ws.onerror = () => {
      setMessages((m) => [
        ...m,
        { type: "error", text: "WebSocket error" },
      ]);
    };
  };
  ...
  const sendTestNotification = async () => {
    try {
      ...
      setMessages((m) => [
        ...m,
        { type: "sent", text: JSON.stringify(res.data) },
      ]);
    } catch (err) {
      console.error(err);
      toast.error(
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      <h3 className="text-lg font-semibold mb-4">
        WebSocket Notifications Tester
      </h3>
