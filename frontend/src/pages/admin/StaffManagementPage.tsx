...
const StaffManagementPage = () => {
  const { isTenantAdmin, isAdmin, tenantId, isSuperAdmin } = useAuth();
  ...
  }, [tenantId, pagination.page, search]);
  ...
  const handleOpenModal = (staffMember: StaffUser | null = null) => {
    if (staffMember) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingStaff) {
        ...
        await tenantApi.updateUser(tenantId, editingStaff.id, updateData);
  ...
  const handleToggleStatus = async (staffMember: StaffUser) => {
    if (!tenantId) return;
    
    try {
      setError("");
      const newStatus = staffMember.is_active ? "inactive" : "active";
      await tenantApi.updateUser(tenantId, staffMember.id, { status: newStatus });
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Quản lý tài khoản nhân viên trong công ty
            </p>
          </div>
          <button
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {staff.map((member) => (
                    <tr key={member.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeColor(member.is_active)}`}>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Mật khẩu {editingStaff && "(để trống nếu không đổi)"}
