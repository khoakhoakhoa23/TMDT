...
const ImportInvoicesPage = () => {
  ...
  const handleDelete = async (ma_hdn) => {
    if (!window.confirm("Bạn có chắc muốn xóa hóa đơn này?")) return;

    try {
      await invoiceApi.deleteImportInvoice(ma_hdn);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800 flex items-center">
          <FaFileImport className="mr-3 text-blue-600" />
          Quản lý Hóa Đơn Nhập
        </h1>
        <Link
          to="/dashboard/import-invoices/create"
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {invoices.length === 0 ? (
              ...
              invoices.map((invoice) => (
                <tr key={invoice.ma_hdn} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    <Link
                      to={`/dashboard/import-invoices/edit/${invoice.ma_hdn}`}
                    ...
                    <button
                      onClick={() => handleDelete(invoice.ma_hdn)}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">
                Chi Tiết Hóa Đơn Nhập: {selectedInvoice.ma_hdn}
              </h2>
              <button
            ...
            <h3 className="text-lg font-semibold mb-3">Danh sách xe nhập:</h3>
                ...
                <tbody>
                  {details.map((detail, index) => (
                    <tr key={index} className="border-t">
                      ...
                      <td className="px-4 py-2 text-right">
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format((detail.don_gia || 0) * detail.so_luong)}
                      </td>
                    </tr>
                  ))}
                  {details.length === 0 && (
